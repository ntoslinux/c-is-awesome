...

{
    ...
    ValidComponentPaths = ConfigData->ValidComponentPaths;
    for (Index = 0; Index < ConfigData->NumValidComponentPaths; Index++) {
...

{
    ...
    BlackListedPaths = ConfigData->BlackListedPaths;
    for (Index = 0; Index < ConfigData->NumBlackListedPaths; Index++) {
    ...
    )
{
    ...
    _wcslwr(Component);
    if (IsValidComponent(Component, ConfigData) == FALSE) {
        Status = ERROR_INVALID_PARAMETER;
        goto Exit;
    }
    ...
    BinPtrRet = calloc(BinLength, sizeof(WCHAR) + 1);
    StringCchCopyNW(BinPtrRet, MAX_PATH, BinPtr, BinLength);
    _wcslwr(BinPtrRet);
    if (IsBlackListedPath(BinPtrRet, ConfigData) != FALSE) {
        Status = ERROR_INVALID_PARAMETER;
        goto FreeAlloc;
    }
    ...
    PdbPtrRet = calloc(PdbLength, sizeof(WCHAR) + 1);
    ...
    StringCchCopyW(Bin, MAX_PATH, BinPtrRet);
    StringCchCopyW(Pdb, MAX_PATH, PdbPtrRet);
    return Status;
